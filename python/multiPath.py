...
def generate_configs(template_files, common_file, env_files):
    for env_file in env_files:
        env_name = os.path.splitext(os.path.basename(env_file))[0]
        mapping = load_mapping(common_file, env_file)

        for template_path, output_folder in template_files:
            os.makedirs(output_folder, exist_ok=True)
            template_base = os.path.splitext(os.path.basename(template_path))[0]
            output_file = os.path.join(output_folder, f"{env_name}_{template_base}.yaml")
            replace_placeholders(template_path, mapping, output_file)

if __name__ == "__main__":
    # Set paths
    template_files = [
        ("templates/infra.yaml", "outputs/infra"),
        ("templates/deployment.yaml", "outputs/deployments")
    ]
